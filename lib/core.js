...
export function createCodal({ loadAsset } = {}) {
  ...
  const build = async (files, { onStep, signal } = {}) => {
    ...
    for (const command of recipe.steps) {
      ...
      signal?.throwIfAborted();
      const step = await invoke(session, command, manifest.layout.project);
      steps.push(step);
      onStep?.(step);
...
// Diagnostics name files as the caller did, so the virtual project prefix comes off them.
async function invoke(session, { tool, args, captureStdout }, project) {
  const chunks = [];
  ...
  return { tool, args, exitCode, stderr, stdout: captureStdout ? join(chunks) : null };
}

// Only where a path starts is the prefix ours: a caller whose own file sits under `project/source/`
// gets it twice in the virtual path, and removing both would name a file they do not have.
function stripProject(text, project) {
  const escaped = project.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  return text.replace(new RegExp(`(^|\\s)${escaped}/`, 'gm'), '$1');
}
